...
fn wait_for_char(port: &mut Box<dyn serialport::SerialPort>, ack_char: u8) -> u8 {
	...
	match port.read(buf.as_mut_slice()) {
	    ...
	    Err(e) => error_handler(format!("Error: failed to read char: {:?}", e)),
	}
...
fn send_packets(packet_list: &Vec<Vec<u8>>, port: &mut Box<dyn serialport::SerialPort>) {
	...
	if c == NAK {
	    match port.write(packet) {
		Ok(_) => {},
		Err(e) => error_handler(format!("Error: failed to read char for packet {:?}: {:?}", pos, e)),
	    }
	    retry_count += 1;
	    if retry_count == 3 {
		// Something deeper is wrong, give up
		error_handler(format!("Error: failed on packet {:?} after 3 tries, giving up.", pos));
	    }
	} else if c == CAN {
	    // cancel, just exit.
	    pb.finish();
...
pub fn get_file(path: &PathBuf, port: &mut Box<dyn serialport::SerialPort>, direct: &bool) {
	...
	Ok(_) => {},
	Err(e) => error_handler(format!("Error: failed to write initial NAK: {:?}", e)),
    }
